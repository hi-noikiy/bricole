...
class PhotoController extends FrontController
{
    ...
    public function getForm($postIdOrToken)
    {
        ...
        if (getSegment(2) == 'create') {
            ...
            $post = Post::withoutGlobalScopes([VerifiedScope::class, ReviewedScope::class])->where('id', session('tmpPostId'))->where('tmp_token', $postIdOrToken)->with(['pictures'])->first();
        } else {
            $post = Post::withoutGlobalScopes([VerifiedScope::class, ReviewedScope::class])->where('user_id', $this->user->id)->where('id', $postIdOrToken)->with(['pictures'])->first();
        }
